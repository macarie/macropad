...
type TupleOf<T, N extends number> = number extends N
  ...
  : {
      [K in N]: BuildPowersOf2LengthArrays<K, [[never]]> extends infer U
        ? U extends never[][]
          ? Replace<ConcatLargestUntilDone<K, U, []>, T> // eslint-disable-line @typescript-eslint/ban-types
          : never
        : never
    }[N]

export type NumbersTo<N extends number> = Partial<TupleOf<unknown, N>>['length']

export default NumbersTo
